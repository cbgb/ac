...
function
asDateTime($key){$time=$this[$key];return(int)$time===0?NULL:new
...
function
asDate($key,$format=NULL){if($format===NULL){return$this->asTimestamp($key);}elseif($format===TRUE){return$this->asDateTime($key);}else{return$this->asDateTime($key)->format($format);}}}final
class
DibiTranslator
extends
...
as$clause=>$val){$this->clauses[$clause]=&$val;}$this->cursor=&$foo;}}if(!function_exists('array_fill_keys')){function
...
REVISION='a8e83ce released on 2010-01-26';const
